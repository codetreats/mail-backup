...
class MailboxClient:
    ...
    def copy_emails(self, days, local_folder, unsynced_folder, wkhtmltopdf):
        ...
        for num in data[0].split():
            ...
            if self.saveEmail(data, local_folder, unsynced_folder):
                n_saved += 1
            else:
                n_exists += 1
    ...
    def getEmailFolder(self, folder, msg, data):
        if msg['Message-Id']:
            foldername = re.sub('[^a-zA-Z0-9_\-]+', '__', msg['Message-Id'])
        else:
        ...
        return os.path.join(folder, year, foldername)



    def saveEmail(self, data, local_folder, unsynced_folder):
        for response_part in data:
            if isinstance(response_part, tuple):
                ...
                directory = self.getEmailFolder(local_folder, msg, data[0][1])

                if os.path.exists(directory):
                    return False
                
                os.makedirs(directory)
                unsyncedDirectory = self.getEmailFolder(unsynced_folder, msg, data[0][1])
                os.makedirs(unsyncedDirectory)
                ...
                try:
                    ...
                    messageCopy = Message(unsyncedDirectory, msg)
                    messageCopy.createRawFile(data[0][1])
                    messageCopy.createMetaFile()
...
def save_emails(account, options):
    ...
    stats = mailbox.copy_emails(options['days'], options['local_folder'], options['unsynced_folder'], options['wkhtmltopdf'])
    mailbox.cleanup()
